    ...
    <div className='w-full z-10'>
      <div className='flex justify-between items-center pb-4 border-b border-gray-400/25'>
        <div>
          ...
          <h2 className='text-xs text-gray-400'>{selectedItem[0].date_start} - {selectedItem[0].date_end}</h2>
        </div>
        <button type='button' onClick={()=>window.history.back()} className='cursor-none hover:text-red-400 transition-colors' onPointerEnter={()=>handleChangePointerColor(true)} onPointerLeave={()=>handleChangePointerColor(false)}>
          <span className="icon-[ion--close-round] w-8 h-8 " role="img" aria-hidden="true"></span>
        </button>
      </div>
      <article className='flex justify-between pt-4'>
          ...
          <div className='w-full flex justify-center'>
            {
              ...
              (<Image
                ...
                className={`${selectedItem[0].id === "expense" ? "w-1/2" : "w-full"} border-2 transition-opacity opacity-0 border-gray-700 !z-50`}
        ...
        <section className='w-1/3 pt-4 px-6 flex flex-col gap-y-4'>
          <p className='text-md' dangerouslySetInnerHTML={{__html:selectedItem[0].description}}/>
          {
            selectedItem[0].github_link &&
            <div className='flex gap-x-1 items-center'>
              <a href={selectedItem[0].github_link} target='_blank' className='cursor-none text-blue-500 hover:underline' onPointerEnter={()=>handleChangePointerColor(true)} onPointerLeave={()=>handleChangePointerColor(false)}>Ver repositorio </a>
              <span className="icon-[tabler--external-link]" role="img" aria-hidden="true"></span>
            </div>
          }
          {
            selectedItem[0].link &&
            <div className='flex gap-x-1 items-center'>
              <a href={selectedItem[0].link} target='_blank' className='cursor-none text-blue-500 hover:underline' onPointerEnter={()=>handleChangePointerColor(true)} onPointerLeave={()=>handleChangePointerColor(false)}>Ir al sitio </a>
              <span className="icon-[tabler--external-link]" role="img" aria-hidden="true"></span>
            </div>
          }
